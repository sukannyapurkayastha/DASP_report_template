...
This module provides functionality to train a BART model (bart-large) 
...
def preprocess_function(examples: Dict[str, List[str]]) -> Dict[str, List[int]]:
    ...
    labels = tokenizer(
        ...
        padding="max_length",  # Explicitly set padding to 'max_length'
        truncation=True,       # Enable truncation
        text_target=targets,   # Use text_target instead of as_target_tokenizer
    )
...
def compute_metrics(eval_pred: Tuple[List[List[int]], List[List[int]]]) -> Dict[str, float]:
    ...
    # Ensure labels are a NumPy array for efficient processing
...
def main():
    ...
    model_name = "facebook/bart-large"  # BART large model
    tokenizer = BartTokenizerFast.from_pretrained(model_name, use_fast=True)
    ...
    # **Forcefully Set decoder_start_token_id to bos_token_id (0)**
    model.config.decoder_start_token_id = tokenizer.bos_token_id
    logger.info(f"Set model decoder_start_token_id to tokenizer.bos_token_id: {model.config.decoder_start_token_id}")
