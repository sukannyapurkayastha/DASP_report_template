...
A script that:
1) Loads a previously fine-tuned T5 model from ./models/t5
...
def load_T5_model(model_dir: str = "./models/t5"):
    """
    Load T5 model previously trained by train_T5.py
    ...
    model_dir : str, optional
        DESCRIPTION. The default is "./models/t5".
    ...
    """
    try: 
        logger.info(f"Loading T5 model from: {model_dir}")
        tokenizer = T5Tokenizer.from_pretrained(model_dir)
        model = T5ForConditionalGeneration.from_pretrained(model_dir)
        
    except:
        logger.info("no pretrained models found. Download them...")
...
def predict(
    ...
    #   - You can also add other generation parameters (num_beams, etc.)
