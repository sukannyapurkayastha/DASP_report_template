...
with open('backend/request_classifier/data/prep/label_map.yaml', 'r') as f:
...
def do_train(tokenizer, model, loss_fn, data_dir, category):
  ...
  for epoch in range(EPOCHS):
    ...
    print(f"Training - Accuracy: {train_acc:.4f}, F1 Score: {train_f1:.4f}, Loss: {train_loss:.4f}")
...
def main():
  ...
  num_classes = len(LABEL_MAP['label_name_map'][args.category])
